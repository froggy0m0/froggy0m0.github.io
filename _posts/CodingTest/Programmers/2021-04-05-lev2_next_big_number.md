...
def solution(inp):
    inp = format(inp,'b')
    if inp.count('1') == len(inp) or inp.count('1') == 1:       #ex)1111, or 1000               
        return int(inp,2) + int(2**(len(inp)-1))
    elif inp[inp.find('0'):].find('1') == -1 : #ex)1100, 11100000  
       	#0을찾고 0이시작 되는인덱스에서 마지막인덱스까지 1이없다면
...
_이전에 짜놨던코드 ... 위코드보다 가독성이 떨어지지만 연산속도는 더빠르다..(맨위 코드는 `1000조` 정도의 연산부터 아주 약간의 연산시간이 필요했다 . 0.03초 ......해당코드는 연산속도가없는것같다)_

_테스트케이스로는 모두통과한걸로 나오지만 정확성에서는 살짝 의문이든다._
